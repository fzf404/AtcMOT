...
def eval_seq(opt, dataloader, data_type, result_filename=None, save_dir=None, show_image=False, frame_rate=10, use_cuda=True):
    if save_dir:
    ...
    for i, (path, img, img0) in enumerate(dataloader):
        ...
        # 发送atc帧信息
        rflag = atcs.send_info(opt.device_id, opt.url,
                               online_ids, online_tlwhs, opt.input_stream)

        if rflag and i % opt.check_setp == 0:
            person_mot.handle_crime(opt.device_id, opt.url,
                                    opt.trigger, online_ids, online_tlwhs, img0)
